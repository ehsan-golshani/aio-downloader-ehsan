...
- file-base64/ برای فایل‌های کوچک (<1MB) - فقط Base64 خام
"""
...
def download_and_save_file(url, channel_name, post_id, media_type, original_filename):
    ...
    try:
        ...
        if is_media:
            ...
            print(f"    📁 Media -> media/ ({content_length} bytes)")
            ...
            # ذخیره فایل Base64 خام (بدون HTML اضافی)
            base64_data = base64.b64encode(resp.content).decode('utf-8')
            base64_path = target_dir / f"{final_filename}.base64"
            base64_path.write_text(base64_data, encoding='utf-8')
            print(f"    📁 Base64 saved: {final_filename}.base64")
...
async def process_channel(page, channel_name, state):
    ...
    print(f"   📥 Downloading {len(new_messages)} messages...")
    ...
    print(f"   ✅ Added {len(new_messages)} messages -> {len(pages)} pages")
